...
public class DriverController : ControllerBase
{
    ...
    [HttpPost("UpdateTransportationNumbers")]
    [Authorize(Roles = "driver")]
    public async Task<ActionResult<ApiResponseModel<bool>>> UpdateTransportationNumbers([FromBody] UpdateTransportationNumbersRequestModel model)
    {
        // Get the current user ID from the token
        var userId = HttpContext.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (string.IsNullOrEmpty(userId))
        ...
        // For security, verify the driver is updating their own transportation numbers
        var driverProfile = await _driverService.GetDriverProfileById(userId);
        if (driverProfile.Data?.Id != model.DriverId)
        {
            return ApiResponseModel<bool>.Fail("You can only update your own transportation numbers", StatusCodes.Status403Forbidden);
        }

        var response = await _driverService.UpdateTransportationNumbers(model);
        return StatusCode(response.StatusCode, response);
    }
}